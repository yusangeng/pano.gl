...
| `src/core/constants.ts` | 从 JSON 导出带类型的常量，以及 `kind → 上传值` 的映射函数 |
| `src/core/types.ts` | `CameraState` / `Projection` / `TextureProjection` / `SourceState` |
...
- Create: `test/support/baseline.ts`
...
```ts
/*
 * Readers for the v0.2.2 fixtures P0 captured.
 *
...
/**
 * `__dirname` is `test/support` under vitest and under Playwright's runner
 * alike, both of which execute the TypeScript source rather than a build.
 */
...
export function readCaptureDoc (camera: string, stateId: string): CaptureDoc {
  const doc = JSON.parse(
    readFileSync(path.join(fixtureRoot, camera, `${stateId}.uniforms.json`), 'utf8')
  ) as { state: CapturedState, frames: UniformWrite[][] }
...
/** A capture's metadata plus its decoded PNG. */
 ...
 *
 * The PNG is top-down, like every image tool, and the GL origin is bottom-up --
 * P0's probe flipped the rows before encoding. A renderer that reads its
 * framebuffer row 0 first is therefore already in the same orientation, and a
 * comparison needs no flip. Flipping here would silently invert the gate.
 */
export function loadCapture (camera: string, stateId: string): Capture {
  return {
    ...readCaptureDoc(camera, stateId),
    png: PNG.sync.read(readFileSync(path.join(fixtureRoot, camera, `${stateId}.png`)))
  }
}
```
