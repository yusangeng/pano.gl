...
- [x] **Step 5: 抽出 baseline 读夹具的公共模块（拆成纯函数 + Node loader 两个文件）**
...
- [x] `npm run test:coverage` 通过，`src/core/**` 分支覆盖 ≥90%
- [x] `matrix-baseline.test.ts` 的 17 条对拍**全绿**（这是本阶段的核心验收）
- [x] `npm run gen:shaders -- --check` 退出码 0
- [x] `src/core/` 里没有任何 `import` 指向 `renderer/` / `media/` / `interaction/` / `viewer/`
- [x] `src/core/` 里没有任何 `window` / `document` / `navigator` 引用
