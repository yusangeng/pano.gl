...
### C1 的灵敏度含义（红网为什么是 quarter-shift 而不是 360° 往返）

- 旧（`/4`，度减自弧度）：姿态每 8π ≈ 25.13° 转一圈，14.3 倍（45/π）过敏。拖一屏宽（`classifyDrag` 产出 360° 姿态）≈ 画面转 14.3 圈。
- 新：姿态 360° = 恰一圈，与 linear 一致。
- **可执行断言（Task 1 实测后修正）**：`rotate(0, 360)` 往返是公式盲的绿钉（姿态 wrap 进 [0, 360)，落在恰好 0，两公式同读 0），不是判别网；C1 的行为级红网是 quarter-shift 对比（quarter(x, y) === home((x - W/4) mod W, y)，旧树实测红在 worst=171，新树 worst=0）。
...
- [x] **Step 1: unit camera-controller——翻转四条、新增两条**
  ...
  3. `'zoom moves by the delta, in the direction the delta asks for'`（:197）：起点 `{ zoom: 0.6 }`，`zoom(0.5)` 后期望 `toBe(0.6 / 1.5)`（写成表达式：与实现除出的结果逐位相同——十进制字面量 `0.4` 是另一个 double，比除法结果大一个 ulp；原 0.5/1.5 是循环小数，换成整除组合）。注释同步：正 delta 是放大，值变小。
...
- [x] **Step 2: unit reference——`exact output pins` 六条期望值按新公式重算**
...
- [x] **Step 4: integration photo——新增灵敏度网（落两条，红预测已修正）**

  在 `'rotate() moves the picture on a non-linear camera'` 两半之后插入两条。**原计划的红预测错了**：姿态在每次 rotate 上 wrap 进 [0, 360)，`90 + 270` 落在恰好 0，而 lngOffset(0) 在新旧两种公式下都读 0——round trip 是**公式盲的绿钉**，不是 C1 的判别网。落地的两条：

  1. `'a full 360° turn on a non-linear camera returns to the picture it started from'`——绿钉：守 wrap/累积不丢角度（`90° turn did not move` 半边 > 2，`did not come home` 半边 ≤ 2，两树皆绿）。
  2. `'a 90° pose turns the picture by exactly a quarter of its width'`——**C1 真正的行为级红网**：1:1 读回，断言 quarter(x, y) === home((x - W/4) mod W, y)，worst ≤ 2。旧树实测红在 **worst=171**（/4 偏移在此姿态减 22.5 弧度 ≈ 209°，远非四分之一圈）；新树实测 **worst=0**（逐位相等）。

- [x] **Step 5: 各自跑红，确认红且红因正确**

  Run: `npx vitest run --project unit test/unit/camera-controller.test.ts -t 'zoom'`
  Expected: **FAIL** ×N——linear-fov 两条（现为 no-op，fov 不动）、clamp 方向两条（旧公式放大顶格在 1）、方向数值一条（0.6×1.5=0.9 ≠ 0.6/1.5）。同值守卫一条**保持绿**（`zoom(-0.5)` 旧公式 1×0.5=0.5 ≠ 1 会置脏——此条也红，红因是 no-op 失效，同样正确）。

  Run: `npx vitest run --project unit test/unit/reference.test.ts`
  Expected: **FAIL** ×6（pins 族），红因全部是 `u` 期望值不符。

  Run: `npx vitest run --project integration user-story-video -t 'zoom'`
  ...
  Run: `npx vitest run --project integration user-story-photo -t '90° pose'`
  Expected: **FAIL**——quarter-shift 网红在 **worst=171**（/4 偏移在 90° 姿态减 22.5 弧度 ≈ 209°，非四分之一圈）。原预测的 `360° round trip` 红是错的：姿态 wrap 到恰好 0，两公式同读 0，该条为公式盲绿钉（Step 4 已修正）。
...
- [x] **Step 1: camera-controller——常量与 zoom() 重写**
...
- [x] **Step 2: reference.ts——lngOffset 与两段大注释**
...
- [x] **Step 3: 两个 shader 同步（同一提交，门 C 红线）**
...
- [x] **Step 4: viewer.ts TSDoc + CLAUDE.md 段落**
...
- [x] **Step 5: 全部红网转绿 + 全量 unit**

  Run: `npx vitest run --project unit test/unit/camera-controller.test.ts test/unit/reference.test.ts`
  Expected: 全绿（含未动的 `output range` / `baseline.test.ts` / 同值守卫 / 零 delta / 非有限值族）。

  Run: `npx vitest run --project integration user-story-video user-story-photo`
  Expected: 全绿（quarter-shift 网 `a 90° pose turns the picture by exactly a quarter of its width` 的 worst ≤ 2 此刻必须绿——实测量 worst=0；`360° round trip` 为公式盲绿钉，两树皆绿）。

  Run: `npm run test:coverage`
  Expected: 四门槛 ≥90% 全过（新分支 `zoom()` 两路 + 负分母路径均有网）。

- [x] **Step 6: 提交（红网与修复同一提交）**
...
- [x] **Step 3: 变异抽查（/tmp git-archive 沙箱，具名击杀才算数）**
  ...
  1. **lng 加回 `/4`**（只改 wgsl 一处）：`reference` 的 pins 族不会红（它们测 CPU 侧），photo 的 `360° round trip` 也不会红（公式盲：wrap 到 0）——杀手必须是**门 C**（cross-backend 像素比对：wgsl 与 glsl 公式分叉）与 photo 的 quarter-shift 网 `a 90° pose turns the picture by exactly a quarter of its width`。若门 C 未红，报告并停手——那是门 C 失效，比变异存活严重。
