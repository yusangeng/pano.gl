...
5. **两个素材 fixture 必须先存在：`public/fixtures/panorama.png`（等距柱状，四象限异色）与 `public/fixtures/clip.mp4`（每一帧同样四象限分得开）。** 本计划与 P4 的所有集成测试都从 `/fixtures/...` 取它们，由 Playwright 配置里的 `vite` dev server（P1）从 `public/` 提供 —— 那是 Vite 的 `publicDir` 默认值，没有任何配置代码。**它们由 P4 的 Task 2 Step 7 产生并提交**，所以 P4 合并进主分支之后它们就在；动手前先 `ls public/fixtures/` 确认，缺了就说明 P4 那一卡没做完，不要在本计划里补造。
...
**另一条同样重要的规则：测试里直接写 `window.__panoTest`，不要 `as unknown as` 再抄一遍签名。** P3 已把 `demo/` 收进两套 tsconfig 的 program，全局 `Window.__panoTest` 声明在测试侧是通的。
...
import type { Viewer, ViewerEvents } from '../../src/viewer/viewer'
...
export interface ViewerTestHooks {
  ...
  /**
   * The viewer the last create call returned.
   *
   * Needed because a gesture has to be driven by the test's own mouse, not from
   * inside `evaluate` -- and then the readback happens in a second `evaluate`,
   * where the viewer would otherwise be out of scope. The create calls return the
   * viewer as well, for a test that wants to assert on it directly.
   */
  ...
  /** A readback of the viewer's canvas. Synchronous. */
  ...
  /** The `<video>` element. Also starts counting listeners on it -- see below. */
  ...
  /**
   * Starts recording `type` on `viewer`.
   *
   * Call it BEFORE the gesture that should trigger the event. That ordering is
   * the whole reason this exists: a gesture is driven by the test's own mouse,
   * i.e. from outside `evaluate`, so the listener has to be installed in one
   * `evaluate` and read in the next -- and there is no closure that spans the
   * two. A field on `window` would work too, at the cost of a cast in every test
   * that needs one; this is that, with the cast in a single place.
   *
   * Re-recording the same type discards what was recorded before.
   */
  record (viewer: Viewer, type: keyof ViewerEvents & string): void
  /** How many events `record` has seen since it was last called for `type`. */
  eventCount (type: keyof ViewerEvents & string): number
...
let lastViewer: FramelessImageViewer | FramelessVideoViewer | undefined
...
function adjust (target: EventTarget, by: number): void {
  tally.set(target, Math.max(0, (tally.get(target) ?? 0) + by))
}

/*
 * Listener accounting, with abort-signal awareness.
 *
 ...
 *
 * The signal handling is not a refinement, it is required for the count to mean
 * anything: `VideoSource` binds its DOM listeners with one `AbortSignal` and
 * drops all of them by aborting it, and an aborted signal removes its listeners
 * internally without ever calling `removeEventListener`. A tally that wrapped
 * only add/remove would report a disposed video source as fully bound.
 */
...
function readCanvas (viewer: Viewer): Pixels {
  ...
  // belongs to the backend, and a canvas' image data is the one readback that
  // does not depend on backend internals. Downscaled to 64x32 on purpose --
  // these tests compare frames, they do not grade them.
  scratch.width = 64
  scratch.height = 32
...
/*
 * Event recording, so a listener can go on before the gesture that should
 * trigger it.
 *
 * The obvious shape -- attach the listener inside `evaluate`, send the gesture,
 * assert -- does not work here: the gesture has to come from the test's own
 * mouse, so the listener is installed in one `evaluate` and read in the next,
 * and nothing spans the two. A field on `window` would do the same job with a
 * cast in every test; this has the cast once.
 */
const records = new Map<string, unknown[]>()

function record (viewer: Viewer, type: keyof ViewerEvents & string): void {
  const seen: unknown[] = []
  records.set(type, seen)
  viewer.on(type, (event) => { seen.push(event) })
}

function eventCount (type: keyof ViewerEvents & string): number {
  return records.get(type)?.length ?? 0
}
...
function videoElement (viewer: Viewer): HTMLVideoElement {
  ...
  // The video element is created detached and never appended, so it is not
  // inside the container and `watch()` cannot have found it. Asking for it is
  // what puts it under the listener count.
  watched.add(el)
  return el
}
 ...
 *
 * `zoom: 1` is the maximum the controller allows, which is the legacy default.
 */
const projections = {
...
async function createOne<T extends Viewer> (
  ...
  // After create(), not before: the canvas exists only once the viewer has it,
  // and the canvas is the element whose listeners are being counted.
  watch(el)
...
const hooks: ViewerTestHooks = {
  ...
  get viewer () {
    if (!lastViewer) throw new Error('no viewer has been created yet')
    return lastViewer
  },
  probe,
...
4. **事件要能「先挂监听、后做手势」。** `record` / `eventCount` 存在的唯一理由是时序：手势必须由测试自己的鼠标发（下一节解释为什么），于是监听器挂在一次 `evaluate` 里、计数读在另一次里，中间没有任何闭包可以传递。**不要为了省这两个出口而在测试里挂 `window.__xxx` 全局** —— 那会让每一条手势测试都要写一次类型强转。

**手势不在这里，因为合成事件驱动不了它。** `InputController` 在 `pointerdown` 里调 `setPointerCapture`，而它对「不是活跃指针」的 `pointerId` 会抛 `NotFoundError` —— 用 `dispatchEvent(new PointerEvent(...))` 造出来的事件永远不是活跃指针。处理器在记录指针之前就抛了，pan 一次都不会发生，而失败表现是「像素没变」，看上去像相机坏了。所以手势由测试用 Playwright 的真实鼠标驱动，读回则靠 `t.viewer` 这个句柄跨 `evaluate` 传递：
 ...
 *
 * `InputController` calls `setPointerCapture` on pointerdown, and that throws
 * `NotFoundError` for a pointer id that is not active -- which a JS-constructed
 * `PointerEvent` never is. The handler returns before it records the pointer, so
 * the gesture does nothing at all and the test sees an unchanged image.
 *
 * One viewer per page, so `.pano-canvas` is unambiguous.
 */

async function centreOf (page: Page): Promise<{ x: number, y: number }> {
  const box = await page.locator('.pano-canvas').boundingBox()
  if (!box) throw new Error('the viewer canvas has no box')
  return { x: box.x + box.width / 2, y: box.y + box.height / 2 }
}

/** Drags across the viewer's canvas, in CSS pixels. */
export async function dragCanvas (page: Page, dx: number, dy: number): Promise<void> {
  const { x, y } = await centreOf(page)
  await page.mouse.move(x, y)
  await page.mouse.down()
  // Stepped rather than one jump: the recogniser emits a delta per move, and a
  // single move would not exercise accumulation.
  for (let step = 1; step <= 4; step++) {
    await page.mouse.move(x + (dx * step) / 4, y + (dy * step) / 4)
  }
  await page.mouse.up()
}

/**
 * Scrolls over the viewer's canvas.
 *
 * Positive `deltaY` is a scroll down, which `classifyWheel` turns into a
 * negative zoom step -- zoom OUT. That matters at the default: the controller
 * clamps zoom to at most 1, so scrolling up from the default is a no-op and a
 * test written that way would assert nothing.
 */
export async function wheelCanvas (page: Page, deltaY: number): Promise<void> {
  const { x, y } = await centreOf(page)
  await page.mouse.move(x, y)
...
1. **`media-load` 到了不等于画面出来了。** 事件来自媒体的 DOM 事件，而渲染要等循环的下一次 tick 看到版本号变化。**读完事件就等 2 帧再读像素**（`await t.waitForFrames(2)`），否则读到的可能是上一帧，甚至什么都没有。
2. **`readCanvas` / `canvasSize` / `countNonBlack` / `maxDiff` / `projection` / `container` / `videoElement` 都是同步的。** 写 `await` 不会错，但会让人以为它们是异步的 —— 不要写。
3. **`waitFor` 要在 `await` 别的东西之前挂上去。** `autoplay` 的视频会在 `media-load` 之前或同时就发出 `media-play`，所以「先 `await` 加载、再等播放」这种写法等的是一个已经过去的事件，结果是超时 5 秒然后断言失败。两条一起等：先拿到两个 promise，再逐个 `await`。
4. **手势用 `test/integration/support/gestures.ts` 里的真实输入，不要合成事件。** 原因在「测试入口需要导出什么」最后一节：合成 `PointerEvent` 会让 `setPointerCapture` 抛 `NotFoundError`，手势静默失效。**跨 `evaluate` 的读回用 `t.viewer`；手势要断言事件，就在手势之前调 `t.record(viewer, 'rotate')`**，之后用 `t.eventCount('rotate')` 读 —— 原因见第 3 条，监听器挂晚了等于没挂。

- [ ] **Step 1: 后端环境掩码**

`test/integration/support/backend.ts`：

```ts
import { expect, type Page } from '@playwright/test'

/*
 * Environment masks for the backend decision.
 *
 * `describeCapabilities` looks at `navigator.gpu` and at whether a WebGL2
 * context can be created, so a test that wants a different branch has to change
 * one of them before the page's modules run -- which is why every helper here
 * takes a page that has NOT navigated yet and goes through `addInitScript`.
 */

/**
 * Makes `navigator.gpu` absent for the next navigation.
 *
 * `delete Navigator.prototype.gpu` rather than assigning `undefined`: the
 * decision reads `'gpu' in navigator`, and a property that exists and is
 * `undefined` still makes that true. Deleting the prototype's property is the
 * only way to make the feature genuinely absent from the page's point of view --
 * and deleting it from the instance would be a no-op, because there is no own
 * property to delete.
 *
 * @param page - A page that has not navigated yet.
 */
export async function maskWebGPU (page: Page): Promise<void> {
...
/**
 * Makes WebGL2 unavailable for the next navigation.
 *
 * For P6, which needs the case where the fallback cannot be used either. There
 * is nothing to delete here -- `getContext` is a method -- so the method is
 * wrapped and made to return null for that one context id.
 *
 * @param page - A page that has not navigated yet.
 */
export async function maskWebGL2 (page: Page): Promise<void> {
  await page.addInitScript(() => {
...
/**
 * Asserts the page really is running on the WebGL2 fallback.
 *
 * Every test that masks WebGPU should call this before asserting anything else.
 * Without it, a mask that silently stopped working would leave the test passing
 * while it exercised the WebGPU path -- the same class of silent no-op the
 * `gpuPage` guard exists to prevent.
 */
export async function expectWebGL2Only (page: Page): Promise<void> {
  const caps = await page.evaluate(async () => await window.__panoTest.probe())
  expect(caps.backend).toBe('webgl2')
}
```

- [ ] **Step 2: US1 —— 看一张全景照片并拖动**

`test/integration/user-story-photo.test.ts`：

```ts
import { test, expect } from './support/fixtures'
import { dragCanvas } from './support/gestures'
...
test.describe('US1: view a 360 photo and look around', () => {
  test('renders the image and reports load', async ({ gpuPage }) => {
    const r = await gpuPage.evaluate(async () => {
      ...
      await t.waitForFrames(2)
      const pixels = t.readCanvas(viewer)
      viewer.dispose()
    ...
    // something on the canvas. The bound is deliberately loose: `countNonBlack`
    // is per-channel (`r > 8 || g > 8 || b > 8`) and every quadrant of the
    // fixture is bright in at least one channel, so a frame that arrived reads
    // near 1.0 while one that never did reads 0. 0.2 sits far from both, which
    // keeps this a test of "did anything arrive" rather than an accidental
    // precision test of the projection.
    expect(r.nonBlack).toBeGreaterThan(0.2)
  })

  test('dragging rotates the camera and changes the image', async ({ gpuPage }) => {
    const before = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createImageViewer({ src: '/fixtures/panorama.png' })
      await t.waitFor(viewer, 'media-load', 5000)
      await t.waitForFrames(2)
      // Before the drag, not after: a listener attached afterwards would only
      // ever see the next gesture, and the assertion would read 0 for a reason
      // that has nothing to do with the pan.
      t.record(viewer, 'rotate')
      return t.readCanvas(viewer)
    })
    // Real mouse input: see support/gestures.ts for why a synthetic
    // PointerEvent cannot drive this.
    await dragCanvas(gpuPage, 200, 0)
    const after = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      await t.waitForFrames(2)
      const result = { pixels: t.readCanvas(t.viewer), rotations: t.eventCount('rotate'), pose: t.viewer.cameraOptions.pose }
      t.viewer.dispose()
      return result
    })
    expect(after.rotations).toBeGreaterThan(0)
    // A horizontal drag moves the longitude, and by a lot more than rounding
    // would explain: the viewer converts pixels to degrees against the frame
    // size, so 200 CSS pixels on a 400-wide container is half the frame.
    expect(after.pose?.povLongitude).not.toBe(0)
    expect(await gpuPage.evaluate(
      ([a, b]) => window.__panoTest.maxDiff(a, b), [before, after.pixels]
    )).toBeGreaterThan(2)
  })
  ...
  test('PTZ = false leaves the image static', async ({ gpuPage }) => {
    ...
    await dragCanvas(gpuPage, 200, 200)
    const after = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      await t.waitForFrames(3)
      const result = { pixels: t.readCanvas(t.viewer), rotations: t.eventCount('rotate') }
      t.viewer.dispose()
      return result
    })
    expect(after.rotations).toBe(0)
    expect(await gpuPage.evaluate(
      ([a, b]) => window.__panoTest.maxDiff(a, b), [before, after.pixels]
  ...
  test('the image fills a container that changes size', async ({ gpuPage }) => {
    ...
    expect(r.before.width).toBe(400)
    expect(r.after.width).toBe(300)
  })
  ...
  test('renders sharply on a high-DPI display', async ({ gpuPage }) => {
    ...
    expect(r.backing.width).toBe(r.css.width * 2)
    expect(r.backing.height).toBe(r.css.height * 2)
  })
})
```

- [ ] **Step 3: US2 —— 播放全景视频并缩放**

`test/integration/user-story-video.test.ts`：

```ts
import { test, expect } from './support/fixtures'
import { wheelCanvas } from './support/gestures'

test.describe('US2: play a 360 video and zoom', () => {
  test('plays and advances frames', async ({ gpuPage }) => {
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createVideoViewer({
        src: '/fixtures/clip.mp4', autoplay: true, loop: true
      })
      // Both listeners first, both awaits after. With autoplay, `media-play` can
      // arrive while this code is still awaiting `media-load`, and a listener
      // attached afterwards would wait five seconds for a second play that never
      // comes -- see rule 3 above.
      const loaded = t.waitFor(viewer, 'media-load', 5000)
      const played = t.waitFor(viewer, 'media-play', 5000)
      const events = { loaded: await loaded, played: await played }
      await t.waitForFrames(3)
      const a = t.readCanvas(viewer)
      // Wall clock, not frame counts: the video advances on its own schedule and
      // the loop only redraws when its version moved. Half a second is many
      // frames of any ordinary fixture.
      await new Promise(res => setTimeout(res, 500))
      const b = t.readCanvas(viewer)
      viewer.dispose()
      return { loaded: events.loaded, played: events.played, diff: t.maxDiff(a, b) }
    })
    expect(r.loaded).toBe(true)
  ...
  test('a paused video stops drawing', async ({ gpuPage }) => {
    // The loop draws when the source's version changes, and a video advances its
    // version only while it is playing. Without that guard this test is the one
    // that catches "a paused video still burns a full-screen shader at 60Hz".
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createVideoViewer({ src: '/fixtures/clip.mp4', autoplay: true, loop: true })
      await t.waitFor(viewer, 'media-play', 5000)
      viewer.pause()
      await t.waitFor(viewer, 'media-pause', 5000)
      await t.waitForFrames(3)
      const before = t.drawCount
      await new Promise(res => setTimeout(res, 500))
      const after = t.drawCount
      viewer.dispose()
      return { frames: after - before }
    })
    expect(r.frames).toBe(0)
  })

  test('a source swap tears down the old element', async ({ gpuPage }) => {
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createVideoViewer({ src: '/fixtures/clip.mp4', autoplay: true, loop: true })
      await t.waitFor(viewer, 'media-load', 5000)
      const first = t.videoElement(viewer)
      viewer.src = '/fixtures/clip.mp4?second'
      await t.waitForFrames(3)
      const second = t.videoElement(viewer)
      const result = {
        replaced: first !== second,
        oldPaused: first.paused,
        oldSrcRemoved: first.getAttribute('src') === null,
        oldListeners: t.listenerCount(first)
      }
      viewer.dispose()
      // Read AFTER dispose, and that ordering is the test's whole point: the live
      // count includes the new element's listeners, which are supposed to be
      // there until the viewer goes away. Reading it before would assert that a
      // working source has no listeners.
      return { ...result, live: t.liveListenerCount() }
    })
    ...
    expect(r.oldPaused).toBe(true)
    expect(r.oldSrcRemoved).toBe(true)
    expect(r.oldListeners).toBe(0)
  ...
  test('autoplay is muted by default', async ({ gpuPage }) => {
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createVideoViewer({ src: '/fixtures/clip.mp4', autoplay: true })
      const muted = t.videoElement(viewer).muted
      viewer.dispose()
      return muted
    })
    // Every modern browser blocks unmuted autoplay, so the legacy default of
    // playing with sound produced a video that never started, with no error.
    expect(r).toBe(true)
  })

  test('zooming out changes the rendering of a non-linear camera', async ({ gpuPage }) => {
    const before = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createImageViewer({ src: '/fixtures/panorama.png', cameraKind: 'cylindrical' })
      await t.waitFor(viewer, 'media-load', 5000)
      await t.waitForFrames(2)
      t.record(viewer, 'zoom')
      return t.readCanvas(viewer)
    })
    // Scroll DOWN, which zooms out. The controller clamps zoom to at most 1 and
    // the default is 1, so scrolling the other way is a no-op by design and a
    // test written that way would assert nothing.
    await wheelCanvas(gpuPage, 200)
    const after = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      await t.waitForFrames(2)
      const projection = t.viewer.cameraOptions.projection
      const result = {
        pixels: t.readCanvas(t.viewer),
        zooms: t.eventCount('zoom'),
        zoom: projection.kind === 'cylindrical' ? projection.zoom : NaN
      }
      t.viewer.dispose()
      return result
    })
    expect(after.zooms).toBeGreaterThan(0)
  ...
  test('zoom is a no-op for the linear camera', async ({ gpuPage }) => {
    const before = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createImageViewer({ src: '/fixtures/panorama.png' })
      await t.waitFor(viewer, 'media-load', 5000)
    ...
    const after = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      await t.waitForFrames(3)
      const result = { pixels: t.readCanvas(t.viewer), zooms: t.eventCount('zoom') }
      t.viewer.dispose()
      return result
    })
  ...
  test('a disposed viewer rejects play()', async ({ gpuPage }) => {
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createVideoViewer({ src: '/fixtures/clip.mp4' })
      await t.waitFor(viewer, 'media-load', 5000)
      viewer.dispose()
      let message = ''
      try {
    ...
    expect(r).toMatch(/disposed/i)
  })
})
```

- [ ] **Step 4: US3 —— 运行时切换相机模型**
...
test.describe('US3: switch camera models at runtime', () => {
  ...
  for (const kind of ['linear', 'cylindrical', 'planet', 'pannini'] as const) {
    test(`switches to ${kind} and keeps the pose`, async ({ gpuPage }) => {
      ...
      const r = await gpuPage.evaluate(async k => {
        const t = window.__panoTest
        const from = k === 'linear' ? 'cylindrical' : 'linear'
        const viewer = await t.createImageViewer({
        ...
        const pixelsBefore = t.readCanvas(viewer)
        // No `pose` in the options: leaving it out is what the assertion is
        // about. Handing the old pose back would pass even if the setter reset
        // the camera and then applied the pose it was given.
        viewer.cameraOptions = { projection: t.projection(k) }
        await t.waitForFrames(3)
        const result = {
          before: poseBefore,
          after: viewer.cameraOptions.pose,
          kind: viewer.cameraOptions.projection.kind,
          changed: t.maxDiff(t.readCanvas(viewer), pixelsBefore)
        }
        viewer.dispose()
        return result
      }, kind)
  ...
  test('a projection swap does not reset the pose the user dragged to', async ({ gpuPage }) => {
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createImageViewer({ src: '/fixtures/panorama.png' })
      await t.waitFor(viewer, 'media-load', 5000)
      ...
      viewer.cameraOptions = { projection: t.projection('planet') }
    ...
    expect(r.after?.povLongitude).toBe(200)
  })

  test('the exported union names exactly the kinds the factory accepts', async ({ gpuPage }) => {
    // There is nothing to assert about an unknown kind at runtime -- it cannot
    // be written down, because `Projection` is a closed union -- so this pins
    // the half that IS observable: the four kinds the viewer accepts are exactly
    // the four the public union names, in both directions. A fifth kind added to
    // one and not the other is the failure this catches.
    const kinds = await gpuPage.evaluate(() =>
      (['linear', 'cylindrical', 'planet', 'pannini'] as const).map(k => window.__panoTest.projection(k).kind)
    )
    expect(kinds).toEqual(['linear', 'cylindrical', 'planet', 'pannini'])
  })
})
```
...
test.describe('US4: media fails to load', () => {
  test('a 404 image emits media-error and does not throw', async ({ gpuPage }) => {
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      // Constructing succeeds: the URL is well-formed, the fetch is what fails.
      const viewer = await t.createImageViewer({ src: '/fixtures/does-not-exist.png' })
      const errored = await t.waitFor(viewer, 'media-error', 5000)
      await t.waitForFrames(2)
      const pixels = t.readCanvas(viewer)
      const nonBlack = t.countNonBlack(pixels)
      // The viewer is still usable, so the teardown is the test's job, not the
      // error's. Asserted here because a viewer that disposed itself on a failed
      // source would take the whole application down with one bad URL.
      const aliveBeforeDispose = !viewer.isDisposed
      viewer.dispose()
      return { errored, nonBlack, aliveBeforeDispose, disposed: viewer.isDisposed }
    })
    expect(r.errored).toBe(true)
    // A failed source leaves an empty canvas, not a stale frame and not an
    // exception. Empty rather than "the background colour": the backend draws
    // nothing at all for a null source, so the canvas keeps the transparent
    // black it was created with.
    expect(r.nonBlack).toBeLessThan(0.1)
    expect(r.aliveBeforeDispose).toBe(true)
    expect(r.disposed).toBe(true)
  })

  test('an undecodable image emits media-error', async ({ gpuPage }) => {
    // A 200 response whose body is not an image: the fetch succeeds and the
    // decode fails, which is a different path from a 404 and the one a
    // misconfigured CDN produces.
    await gpuPage.route('**/fixtures/not-an-image.png', route => route.fulfill({
      status: 200,
    ...
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createImageViewer({ src: '/fixtures/not-an-image.png' })
      const errored = await t.waitFor(viewer, 'media-error', 5000)
      viewer.dispose()
      return errored
    })
  ...
  test('a video with a bad codec emits media-error', async ({ gpuPage }) => {
    await gpuPage.route('**/fixtures/bad-codec.mp4', route => route.fulfill({
      status: 200,
      contentType: 'video/mp4',
      body: 'this is not an MP4'
    }))
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createVideoViewer({ src: '/fixtures/bad-codec.mp4', autoplay: true })
      const errored = await t.waitFor(viewer, 'media-error', 5000)
      viewer.dispose()
      return errored
    })
  ...
  test('the viewer keeps working after a source error', async ({ gpuPage }) => {
    // An error is a property of the source, not of the viewer. The camera must
    // still move and the loop must still draw, or one bad URL would leave the
    // application with a dead object it cannot even navigate.
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createImageViewer({ src: '/fixtures/does-not-exist.png' })
      await t.waitFor(viewer, 'media-error', 5000)
      await t.waitForFrames(2)
      const before = t.drawCount
      viewer.rotate(10, 10)
      await t.waitForFrames(3)
      ...
      viewer.dispose()
      return { drew: after - before, pose }
    })
    expect(r.drew).toBeGreaterThan(0)
    expect(r.pose).toEqual({ povLatitude: 10, povLongitude: 10 })
  })

  test('replacing a broken source recovers', async ({ gpuPage }) => {
    const r = await gpuPage.evaluate(async () => {
      const t = window.__panoTest
      const viewer = await t.createImageViewer({ src: '/fixtures/does-not-exist.png' })
      await t.waitFor(viewer, 'media-error', 5000)
  ...
  test('an empty src throws at construction', async ({ gpuPage }) => {
  ...
  test('the removed frameSize option is refused loudly', async ({ gpuPage }) => {
    ...
    // defence -- the runtime check, for the JavaScript caller whose config
    // object still has this key. A variable is how such an object is spelled in
    // a typed test without a cast.
    const legacyOptions = { src: '/fixtures/panorama.png', frameSize: [2048, 1024] }
    const message = await gpuPage.evaluate(async options => {
      const t = window.__panoTest
      try {
        await t.createImageViewer(options)
        return ''
      } catch (e) {
        return String(e)
      }
    }, legacyOptions)
    expect(message).toMatch(/frameSize/)
  })
})
```

- [ ] **Step 6: US5 —— 没有 WebGPU 的环境**

`test/integration/user-story-no-webgpu.test.ts`：

```ts
import { test, expect } from '@playwright/test'
import { maskWebGPU } from './support/backend'

/*
 * US5 does NOT use the `gpuPage` fixture, and that is the whole point of the
 * story: `gpuPage` asserts a working WebGPU adapter before the test body runs,
 * so a test that needs WebGPU to be MISSING cannot be written on it. This is the
 * one file that uses the base `test` fixture and navigates itself.
 *
 * The mask goes in through `addInitScript`, i.e. before the page's modules run,
 * because `probe()` reads `navigator.gpu` at call time and the decision has to
 * see the masked world from the start.
 */
test.describe('US5: running where WebGPU is unavailable', () => {
  test('probe() reports the situation instead of throwing', async ({ page }) => {
    await maskWebGPU(page)
    await page.goto('/')
    const caps = await page.evaluate(async () => await window.__panoTest.probe())
    // 'none' until P6 lands the WebGL2 backend. When it does, this becomes
    // 'webgl2' -- P6 flips this expectation deliberately, as part of proving the
    // fallback exists. `probe()` must never throw: its whole reason for existing
    // is to be callable before anything is constructed.
    expect(caps.backend).toBe('none')
  })

  test('create() fails with a message naming the backend situation', async ({ page }) => {
    await maskWebGPU(page)
    await page.goto('/')
    ...
    // Until P6: an honest failure, not a black rectangle. The legacy
    // `createProgram` logged and returned null, and the viewer reported success
    // and rendered nothing forever.
    expect(message).toMatch(/no usable rendering backend/i)
  })

  test('an unmasked page still renders, as a control for the mask', async ({ page }) => {
    // The control: the same page, the same test body, no mask. Without it, the
    // two tests above would pass just as well if `addInitScript` had broken the
    // page outright -- a `goto('/')` that never runs the app, or a page whose
    // modules all threw, would satisfy "probe says none" and "create throws" for
...
> **P6 接手这一节时要改两处**：把 `expect(caps.backend).toBe('none')` 改成 `'webgl2'`（并在它之前调一次 `expectWebGL2Only(page)`，`support/backend.ts` 里已经备好），把「create() 失败」改成「create() 成功且画出来了」。**第三个测试不要动** —— 它是掩码的对照组，与后端无关。
>
> **P6 也可能会删掉这个文件**：它的 Task 5 打算让 `chromium-webgl2` 这个 project 把四个 User Story 整体重跑一遍，那时「没有 WebGPU 的环境」就不再是一个独立的故事，而是全量重跑的一个 project。两条路都行，**但不要两条都走** —— 留下这个文件就必须改上面两处，删掉它就必须确认 `playwright.config.ts` 里那个 project 真的覆盖了后端选择这条路径。**按 P6 现在的写法它走的是删掉这条路**（`playwright.config.ts` 用 `testMatch` 点名四个文件、把第五个排除在外，Task 6 再 `git rm` 它），所以接手时先看 P6 的 Task 5/6 落在哪一边，不要两边各改一半。**`support/backend.ts` 无论走哪条都要留着**：P6 的 project 分流要用它那两行 `addInitScript`，不要再抄一份。
...
camera switching, media failure, and the no-WebGPU environment -- the last one
on the base fixture, because the gpuPage guard asserts the very capability that
story is about."
```
...
| `test/integration/support/backend.ts`（`maskWebGPU` / `maskWebGL2` / `expectWebGL2Only`） | P6 的 project 分流与 US5 都用它，**不要再自己造一个 `forceBackend` 选项**：后端不是构造参数，是环境决定的 |
| 五个 User Story 测试 | P6 让它们**在 WebGL2 后端下也全部跑一遍** —— 这是后端替换的验收方式。**P6 必须原样跑，不许为了让它们过而放宽断言**：它们描述的是用户看得见的行为，两个后端都该满足 |
| `demo/test-entry-hooks/viewer.ts` 的 `ViewerTestHooks` | P6 的钩子文件加在自己名下，**不要改这个文件**；需要新出口就在 `demo/test-entry-hooks/` 里加一个新的，接口靠 `declare module` 合并 |
| `t.record` / `t.eventCount` | 任何「先做手势、后断言事件」的测试都要用它。**P6 的钩子文件不要重新实现一份**，`declare module` 合并出来的接口里已经有了 |
