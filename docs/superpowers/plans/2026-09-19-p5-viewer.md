...
**不要改 P3 的 `test-entry.ts` 去加字段。** 它把 `PanoTestApi` 设计成可以被后续阶段**拓宽**，手段是 TS 的声明合并：钩子文件里写 `declare module '../test-entry'`。P4 和 P6 各有自己的钩子文件，三个阶段共用一个接口就是三个人改同一行。
