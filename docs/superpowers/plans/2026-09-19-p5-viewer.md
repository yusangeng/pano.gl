...
- [x] **Step 1: 实现 `backend-factory.ts`**
...
- [x] 五个 User Story 全绿，包含各自的异常场景
- [x] `npm run test:unit` 覆盖率过关，`npm run typecheck` 干净
- [x] 静止图片在 1 秒内产生的 draw 次数 < 10；暂停的视频一帧都不再画
- [x] `dispose()` 后：draw 数不再增长、容器里不再有 canvas、公开方法抛异常
- [x] 设备被销毁时 `device-lost` 先发出、再完成释放，且重入安全
- [x] `create()` 在无后端时抛异常，且消息说清是哪个后端不可用；`probe()` **不抛**，把「一个后端都没有」当作一个答案返回
- [x] 换相机模型**保留**姿态 —— 这是与旧版的行为差异，旧版的 setter 会重建相机并回到原点
- [x] 换素材**也保留**姿态：素材是素材，相机是用户的。`cameraOptions` 才是回到原点的方式
- [x] `Backend.onDeviceLost` 真的被用上了 —— 没有它 `device-lost` 事件不可能发出，Task 3 也就无解
- [x] **公开 API 冻结** —— `src/index.ts` 的导出面自此不再变（P6/P7 不改它）
- [x] demo 页挂上真 viewer：`npm start` 打开即渲染、拖动能转（Task 6）
