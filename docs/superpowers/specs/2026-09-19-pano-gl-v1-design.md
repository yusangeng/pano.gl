...
**【核码】** 三层链路的净效果是 `povLongitude / 4`：`CameraState.povLongitude` 是**度**，`theta` 是**弧度**，减掉的是二者的混合量 —— 既不是正确的度转弧度（`* PI / 180`，差约 29 倍；29 ≈ 90/π，是只算 `/2.0` 一层的倍数，三层链路的净系数是 `/4`，实际过敏感倍数是 45/π ≈ 14.3），也不是任何一致的量纲。**修正它会直接改变拖拽灵敏度**，那是一个用户可见的行为变更，不是一次内部重构。
